...
#include <math.h>
#define PI acos(-1)
//Task 05: Verify a triangle, and calculate the square
...
int verify(double side[3]){
    ...
    if(side[0] + side[1] <= side[2] || side[2] - side[1] >= side[0]){
        return 1;
    } else {
        return 0;
    }
...
int main() {
    ...
    while (!(scanf("%lf, %lf, %lf", &a[0], &a[1], &a[2]) == 3 && a[0] > 0 && a[1] > 0 && a[2] > 0)) {
        printf("Invalid input, try again(a, b, c): ");
    }
    if (verify(a) == 0){
        ...
        printf("Invalid triangle. Program will exit now.");
    }
    return 0;
}
